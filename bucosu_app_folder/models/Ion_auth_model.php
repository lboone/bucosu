...
class Ion_auth_model extends My_Model
{
	...
	{
		$this->trigger_events('pre_activate_group');
		...
		if ($return)
		{
			$this->trigger_events(array('post_activate_group', 'post_activate_group_successful'));
			$this->set_message('activate_successful');
		}
		else
		{
			$this->trigger_events(array('post_activate_group', 'post_activate_group_unsuccessful'));
			$this->set_error('activate_unsuccessful');
		}
	 ...
	 * @author Mathew
	 **/
	public function deactivate_group($id = NULL)
	{
		$this->trigger_events('deactivate');
		...
		$return = $this->db->affected_rows() == 1;
		if ($return)
			$this->set_message('deactivate_group_successful');
		else
			$this->set_error('deactivate_group_unsuccessful');
	...
	public function get_users_groups($id=FALSE)
	{
		...
		return $this->db->select('group_id as id')
						->select('group_name as name')
						->select('group_description as desc')
						->select('group_type as type')
						->select('group_type_name as type_name')
						->select('group_level as level')
						->from('v_user_group')
						->where('user_id',$id)
						->get();

/*
		return $this->db->select($this->tables['users_groups'].'.'.$this->join['groups'].' as id, '.$this->tables['groups'].'.name, '.$this->tables['groups'].'.type, '.$this->tables['groups'].'.description, '.$this->tables['groups'].'.level')
...
*/
	...
	{
		$this->trigger_events('group_types');
		...
		$this->response = $this->db->get($this->tables['group_types']);
	...
	public function companies()
	{
		...
		$this->response = $this->db->get($this->tables['companies']);
	...
	{
		$this->trigger_events('company');

		if (isset($id))
		{
			$this->where($this->tables['companies'].'.id', $id);
		}
	...
	**/
	...
	public function set_session($user)
	{
		...
		$id = $user->id;
		$ucag = $this->get_user_can_add_groups_data($id);
		$co = $this->get_company_data($user->company);
		$agrmnts = $this->get_bcs_agreements($co);

		$session_data = array(
		    'identity'             		=> $user->{$this->identity_column},
		    'username'             		=> $user->username,
		    'email'                		=> $user->email,
		    'user_id'              		=> $id, //everyone likes to overwrite id so we'll use user_id
		    'old_last_login'       		=> $user->last_login,
		    'user_can_add_groups'  		=> $ucag,
		    'company'			   		=> $co,
		    'agreements'		   		=> $agrmnts,
		    'associated_companies' 		=> $this->get_associated_companies($co),
		    'screen_locked'		   		=> FALSE,
		);

		$this->session->set_userdata($session_data);
		$this->set_school_districts_schools();
		$this->set_bcs_headings_profiles();

		$this->load->module('user_settings');
		$this->user_settings->update_session_with_settings($id);
	...
	public function login_remembered_user()
	{
		...
		//get the user
		$this->trigger_events('extra_where');
		$query = $this->db->select($this->identity_column.', id, username, email, last_login, company')
	...
	*/
	 ...
	 * @author aditya menon
	*/
	public function create_company($company_data = array())
	{
		// bail if the group name was not passed
		if(!$company_data['name'])
		{
			$this->set_error('Company Name Requered');
			return FALSE;
		}

		// bail if the group name already exists
		$existing_company = $this->db->get_where($this->tables['companies'], array('name' => $company_data['name']))->num_rows();
		...
		$data = array_merge($this->_filter_data($this->tables['companies'], $company_data), $company_data);

		$this->trigger_events('extra_group_set');

		// insert the new group
		$this->db->insert($this->tables['companies'], $data);
		$company_id = $this->db->insert_id();

		// report success
		$this->set_message('Company creation successfull');
		// return the brand new group id
		return $company_id;
	}
	 ...
	 * @author aditya menon
	 **/
	public function update_company($company_id = FALSE, $company_name = FALSE, $additional_data = array())
	{
		...
		if (!empty($company_name))
		{
			...
			// bail if the group name already exists
			$existing_company = $this->db->get_where($this->tables['companies'], array('name' => $company_name))->row();
			if(isset($existing_company->id) && $existing_company->id != $company_id)
			{
				$this->set_error('company_already_exists');
				return FALSE;
			}
		...
		//filter out any data passed that doesnt have a matching column in the groups table
		//and merge the set group data and the additional data
		if (!empty($additional_data)) $data = array_merge($this->_filter_data($this->tables['companies'], $additional_data), $data);


		$this->db->update($this->tables['companies'], $data, array('id' => $company_id));

		$this->set_message('Company Update Successfull');
	...
	**/
	...
	public function unlockscreen(){
		...
		$user = $this->get_by($queryVals, TRUE);

		if (count($user)) {
			$this->session->set_userdata(array('screen_locked' => FALSE));		
			return TRUE;
		}*/
	...
	public function screenlocked(){
		return (bool) $this->session->screen_locked;
	}
	...
	{
		if (is_null($id)) {
			$co = $this->session->company;
			$id = $co->id;
		}
		$this->_table_name = 'v_companies';
		$this->_order_by = 'name';
		$queryVals = array(
			'id' => intval($id),
		);

		$company = $this->get_by($queryVals,TRUE);
		return $company;
	}

	public function set_company_data($id)
	{
		$this->session->set_userdata(array('company'=>$this->get_company_data($id)));
	}

	public function get_user_can_add_groups_data($id)
	{
		$this->_table_name = 'v_user_group_can_add';
		$this->_order_by = 'user_id';
		$queryVals = array(
			'user_id' 		=> intval($id),
			);

		$groups = $this->get_by($queryVals);
		$can_add_groups = array();
		
		$this->_order_by = 'email';
		
		if(count($groups)) {
			$firstTime = TRUE;
			foreach ($groups as $group) {
				if ($firstTime) {
					$this->session->set_userdata(array('user_group_id'=>$group->group_id));
					$this->session->set_userdata(array('user_group_type'=>$group->group_type));
					$this->session->set_userdata(array('user_group_type_name'=>$group->group_type_name));
					$this->session->set_userdata(array('user_group_name'=>$group->group_name));
					$this->session->set_userdata(array('user_group_level'=>$group->group_level));
				} 
					$can_add_groups[$group->can_add_type_name][]=array(
																	'can_add_id' => $group->can_add_id,
																	'can_add_name'=> $group->can_add_name,
																	'can_add_level'=> $group->can_add_level,
																	);	
					
					$firstTime = FALSE;
			}
			return $can_add_groups;
		} else {
			$this->_table_name = 'v_user_group';
			...
			$this->session->set_userdata(array('user_group_name'=>$group->group_name));
			$this->session->set_userdata(array('user_group_level'=>$group->group_level));
			$can_add_groups = array('error'=>'User can"t add new users');
			return $can_add_groups;
		}
	}
	
	public function set_user_can_add_groups_data($id)
	{
		$this->session->set_userdata(array('user_can_add_groups'=>$this->get_user_can_add_groups_data($id)));
	}

	public function get_bcs_agreements($company = NULL)
	{
		if ($company == NULL) {
		...
		if ($company->type) {
			...
			$this->_table_name = 'bcs_agreements';
			$this->_order_by = 'id';
			$id = intval($company->id);

			$co_type = intval($company->type);
			...
			if ($co_type == 2) 
			{
				...
				$agreements = $this->get_by($queryVals);
				if (count($agreements)) {
					foreach ($agreements as $agreement) {
						$co = $this->get_company_data($agreement->sd_id);

						$agreement_data[] = array(
											'agreement_id'		=> $agreement->id,
											'date_started'		=> $agreement->date_start,
											'agreement_with'	=> $co,
											);
					}
	...
	public function set_bcs_agreements($co)
	{
		$this->session->set_userdata(array('agreements' => $this->get_bcs_agreements($co)));
	}

	public function get_associated_companies($currentCompany = NULL)
	{
		if(intval($this->session->user_group_level) == 0) 
		{
			$this->_table_name = 'v_companies';
			$this->_order_by = 'id';	
			$companiez = $this->get();
			if ($companiez) {
				foreach ($companiez as $companyz) {
					$companies[$companyz->type_name][] = array(
														'id' => $companyz->id,
														'name' => $companyz->name
														);
				}
		...
		{	
			if ($currentCompany == NULL) {
				$currentCompany = $this->get_company_data();
			}

			$agreements = NULL;
			$agreements = $this->get_bcs_agreements($currentCompany);
			$companies[$currentCompany->type_name][] = array(
														'id' => $currentCompany->id,
														'name' => $currentCompany->name);
			if ($agreements) {
				foreach ($agreements as $agreement) {
					$companies[$agreement['agreement_with']->type_name][] = array(
																				'id'=>$agreement['agreement_with']->id,
	...
	public function get_school_districts_schools(){
	    $this->load->module('bcs_events_school_district');
	    $esdModule = $this->bcs_events_school_district; 
	    $sd_events = $esdModule->get_users_school_districts_school();
	...
	public function set_school_districts_schools(){
		$sds = $this->get_school_districts_schools();
		if ($sds) {
			$this->session->set_userdata(array('school_districts_schools'=>$sds));	
		}
	}

	public function get_bcs_headings_profiles(){
		$this->load->module('bcs_headings');
		$bcs_h = $this->bcs_headings;
		$headings = $bcs_h->get_headings_profiles_project();
	...
	public function set_bcs_headings_profiles(){
		$bcs_hp = $this->get_bcs_headings_profiles();
		if ($bcs_hp) {
			$this->session->set_userdata(array('bcs_headings_profiles'=>$bcs_hp));	
		}
	}
	 ...
	 *
	 */
	public function filter_users_by_company($users)
	{
		$user_group_type = intval($this->session->user_group_type);
		...
		foreach ($users as $k => $user)
		{
			if ($user_group_type == 3) {
				...
				$f_user_groups = $this->get_users_groups($user->id)->result();	

				if (intval($f_user_groups[0]->type) == 3 || intval($f_user_groups[0]->type) == 4)
				{
					$f_user_company = $this->get_company_data($users[$k]->company);
					$f_user_company_id = intval($f_user_company->id);
					if ($f_user_company_id !== $user_company_id) 
					{
						unset($users[$k]);
					} else {
						$users[$k]->groups = $this->get_users_groups($user->id)->result();	
					}
				} 
				else
				{
			...
			} elseif ($user_group_type == 2) {

				$f_user_groups = $this->get_users_groups($user->id)->result();	
				if (intval($f_user_groups[0]->type) == 2 || intval($f_user_groups[0]->type) == 4)
				{
					$f_user_company = $this->get_company_data($users[$k]->company);
					$f_user_company_id = intval($f_user_company->id);
					if ($f_user_company_id !== $user_company_id) 
				...
				} elseif (intval($f_user_groups[0]->type) == 3) {
					//show school districts / their agreements only	
					$user_agreements = $this->get_bcs_agreements($this->session->company);
					foreach ($user_agreements as $agreement) {
						if (intval($agreement['agreement_with']->id) == intval($user->company)) {
							$users[$k]->groups = $this->get_users_groups($user->id)->result();
							$made_id = TRUE;
							break;
						}
	...
	public function get_all_users_companies(){
		$this->_table_name = 'v_user_company';
		$this->_order_by = 'user_id';
		$this->_primary_key = 'user_id';
		$cos = $this->get();
		if ($cos) {

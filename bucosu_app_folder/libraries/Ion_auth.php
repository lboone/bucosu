...
class Ion_auth
{
	...
	public function __construct()
	{
		...
		$config = array(
			...
			'smtp_port'	=> 465,
			'smtp_user'	=> 'lloydaboone@gmail.com',
			'smtp_pass' => 'Jbakids#3',
		);
	 ...
	 * @author Ben Edmunds
	 **/
	public function is_admin($id=false,$maxLevel=2)
	{
		$this->ion_auth_model->trigger_events('is_admin');

		$admin_group = $this->config->item('admin_group', 'ion_auth');
		$uGroups = $this->get_users_groups($id)->result();
		if (isset($uGroups) && is_array($uGroups) && isset($uGroups[0]->level)){
			$level = intval($uGroups[0]->level);
			if ($level <=$maxLevel ) {
				return TRUE;
			}
		} elseif($this->in_group($admin_group, $id)) {
			return TRUE;
		}
		return FALSE;

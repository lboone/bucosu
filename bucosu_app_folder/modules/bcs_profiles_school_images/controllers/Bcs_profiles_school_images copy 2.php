...
class Bcs_profiles_school_images extends Authorized_Controller {
	...
	public function get_all_images_by_profile_school_id($profile_school_id=null){
		...
		if ($imgs) {
			$psi_images = array();
			foreach ($imgs as $img) {
				$psi_images[] = array('image_src'=>$img_src . '/' . $img->image_src,'image_psi'=>$img->profile_id,'image_title'=>strtoupper($img->image_title) . '<br />Uploaded By: ' . $img->uploaded_by . ', ' . $img->company_name . ' <br />' . date('D, M d Y',strtotime($img->created)));
			}
			return $psi_images;
		} else {
			return null;

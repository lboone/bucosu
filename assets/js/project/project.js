      ...
      if (arr.success)
      {
        ...
        log_the_data();
          ...
          success: function(resp){
            ...
            view_projects_pre_load_setup();
            view_project_pre_load_setup();
          }
        });
      } else {
        alert(arr.msg);
  ...
  $(document).on('change','#school_district_select', function(){
    ...
    $.when(
      ...
      $.get("../../user_settings/update_setting/?setting=project_school_district_name&value="+txt, function(resp){
        ...
        if (on_view_projects_tab())
        {
          //view_projects_updates('#view_projects_table_header','html',txt);
          window.location = 'http://bucosu.com/projects/view_projects';
        } else {
...
/* ------ NEW PROJECT TAB ------*/
  ...
  $(document).on('change','#project_details_profile',function(){
    ...
    $('#how_many_profiles_chosen').html('0');
    clear_all_project_fields(true);
  });
  ...
  $(document).on('click','#search_for_school_profiles',function(e){
    ...
    var pfep  = $('#project_filter_existing_project').val();
      ...
      success: function(resp){
        ...
        { 
          ...
          $.each(arr.msg, function (key,value){
            ...
            cur_p_p = value['profile_projects'];
            ...
            if (pfep !== 'none' && !empty(cur_p_p)){ filterProfile = false;} // If Exclude By Prior Project & This Profile Has A Project, skip this one. 
                    ...
                    if (value2.kind == 'capital_planning_life')
                    {
                      switch(pfrul){
                        case 'five_years_or_less':
                          if ( (Number(value2.answer)<6) && ( !empty(value2.answer) ) ) {
                            found = true;
                          }
                        break;
            ...
            if (found){
                ...
                if (cur_p_i){
                  imgVal = '<tr>';
                  imgVal += '<td>Profile Images</td>';
                  imgVal += '<td>';
                  imgVal += '<div class="panel-group" id="accordion2" role="tablist" aria-multiselectable="true">';
                  imgVal += '  <div class="panel panel-default">';
                  imgVal += '    <div class="panel-heading" role="tab" id="headingTwo">';
                  imgVal += '      <h4 class="panel-title">';
                  imgVal += '        <a role="button" data-toggle="collapse" data-parent="#accordion2" href="#collapseTwo" aria-expanded="true" aria-controls="collapseTwo">';
                  imgVal += '          Expand/Collapse Profile Images';
                  imgVal += '        </a>';
                  imgVal += '      </h4>';
                  imgVal += '    </div>';
                  imgVal += '    <div id="collapseTwo" class="panel-collapse collapse" role="tabpanel" aria-labelledby="headingTwo">';
                  imgVal += '      <div class="panel-body">';
                  imgVal += '       <div class="sectionrow">';
                  ...
                  imgVal = imgVal + '</div></div></div></div></div></td></tr>';
                }

                if (!empty(cur_p_p))
                {
                  if (Array.isArray(cur_p_p))
                  {
                    var cpp_a = '';
                    $.each(cur_p_p, function (key4, value4){
                      cpp_a += '<a href="http://bucosu.com/projects/view_project?id='+ value4.project_id +'">Click to view the Project!</a><br/>';
                    });
                  };
                  cpp_row = '<tr><td>Existing Project:</td><td background="http://bucosu.com/attachments/app/projects/CheckYes.png" style="height: 110px; background-repeat: no-repeat; background-position: 200px center;">'+cpp_a+'</td></tr>';
                } else {
                  cpp_row = '';
                }
                var cur_q_f1 = '';
                cur_q_f1 += '<div class="panel-group" id="accordion1" role="tablist" aria-multiselectable="true">';
                cur_q_f1 += '  <div class="panel panel-default">';
                cur_q_f1 += '    <div class="panel-heading" role="tab" id="headingOne">';
                cur_q_f1 += '      <h4 class="panel-title">';
                cur_q_f1 += '        <a role="button" data-toggle="collapse" data-parent="#accordion1" href="#collapseOne" aria-expanded="true" aria-controls="collapseOne">';
                cur_q_f1 += '          Collapse/Expand Profile Answers';
                cur_q_f1 += '        </a>';
                cur_q_f1 += '      </h4>';
                cur_q_f1 += '    </div>';
                cur_q_f1 += '    <div id="collapseOne" class="panel-collapse collapse in" role="tabpanel" aria-labelledby="headingOne">';
                cur_q_f1 += '      <div class="panel-body">';
                cur_q_f1 += cur_q_f;
                cur_q_f1 += '</div></div></div></div>';
                cur_q_f  =  cur_q_f1;

                $('#bcs_profiles_school_results').append('<!-- begin: #dashboard-c1-p1 --><div id="new_project-c1-p' + cur_s_p.id + '" class="panel panel-default"><div class="panel-heading" style="line-height:15px !important;"><span class="panel-title"><span class="fa fa-calendar"></span>' + cur_s_p.name + '</span><div id="new_project_check_button" class="pull-right"><div class="bs-component"><div class="checkbox-custom mb5"><input type="checkbox" value="' + cur_s_p.id +'" name= "project_profiles[]" id="project_profile_' + cur_s_p.id + '"><label style="line-height: 20px;" for="project_profile_' + cur_s_p.id + '">'+ cur_s_p.name + '</label></div></div></div></div><div class="panel-body"><table class="table table-condensed responsive"><tbody><tr><td>Created On:</td><td>'+cur_s_p.created+'</td></tr><tr><td>Locations:</td><td>'+locs+'</td></tr><tr><td>Notes:</td><td>'+cur_s_p.notes+'</td></tr><tr><td>Alert:</td><td>'+cur_s_p.facilities_alert_description+'</td></tr>'+cpp_row +'<tr><td>BCS Answers</td><td>' + cur_q_f + '</td></tr>' + imgVal + '</tbody></table></div></div><!-- end: #dashboard-c1-p1 -->');
            }
          });

          
          if (empty(filtered_s_p))
          {
            $('#bcs_profiles_school_results').append('<h2>Sorry no profiles matching that criteria, please try again</h2>');
            $('#search_for_school_profiles_results_count').html("0 Profile(s) Found");
          } else {
            var f_l = Object.keys(filtered_s_p).length;

            $('#search_for_school_profiles_results_count').html(f_l + " Profile(s) Found");
          }
  ...
  $(document).on('click','#new_project_btn-generate_project',function(e){
    ...
    if (proj_type == 'bcs_project')
    {
      ...
      $('#event_school_district_id').val(chosen_sd_event.id);
      $('#event_school_id').val(chosen_s.id);
      $('#is_bcs_project').val('1');

      var minYear = 200;
      var totAmt = 0;
      var title = $('#project_details_profile').find('option:selected').html();
      var bcs_qnum = chosen_p['bcs_qnum'];
      var profs = new Array();
    ...
    event.preventDefault();
      ...
      if ($('#project_form').valid()){
        ...
        .done(function(data){
          ...
          if (arr.success)
          {
            window.location.href ='http://bucosu.com/projects/view_projects';

          } else {
            $('#project_form_container .panel-heading').append(arr.msg);
            setTimeout(function() {
              $('.temp_project_message').remove();
            }, 7500);
            var el = $('#project_form_container .panel-heading');
            $('html,body').animate({scrollTop: el.offset().top - 100}, 200, function() {
              el.focus();
            });
          }
  ...
  $("form[id^=project_form]").validate({
      ...
      messages: {
        ...
        year_to_complete: {
          required: 'Year To Complete required.',
          number: 'You must provide a number.'
        },
        cost: {
        ...
        priority: {
          required: 'Prority required'
        },
        purpose_id: {
          required: 'Project Purpose required.'
        },
        type_id: {
          required: 'Project Type required.'
        },
        bcs_quesiton_number: {
          required: 'BCS Question Number rerquired.'
        }
      },
...
/* ------ VIEW PROJECTS FUNCTIONALITY ------*/
  function view_projects_pre_load_setup(){
    if (on_view_projects_tab()){      
      var url = "http://bucosu.com/assets/js/project/view_projects/view_projects.js";
      $.getScript(url, function(){

      });
    }
  }
  function view_projects_updates(sel,action,val){
    if (on_view_projects_tab()){
      switch (action){
        case 'html':
          $(sel).html(val);
...
/* ------ VIEW PROJECT FUNCTIONALITY ------*/
  function view_project_pre_load_setup(){
    if (on_view_project_tab()){      
      var url = "http://bucosu.com/assets/js/project/view_project/view_project.js";
      $.getScript(url, function(){

      });
    }
  }
  function view_project_updates(sel,action,val){
    if (on_view_project_tab()){
      switch (action){
        case 'html':
          $(sel).html(val);
...
/* ------ UTILITY FUNCTIONS ------*/
  /* ------ A FUNCTION THAT CHECKS TO SEE IF WE ARE ON THE VIEW PROJECTS TAB ------*/
  function on_view_projects_tab(){
    return !$('#view_projects_tab_container').hasClass('hidden');
  }
  /* ------ A FUNCTION THAT CHECKS TO SEE IF WE ARE ON THE VIEW PROJECTS TAB ------*/
  function on_view_project_tab(){
    return !$('#view_project_tab_container').hasClass('hidden');
  }
  ...
  function reset_default_fields_and_values(clearChosen){
    ...
    $('#search_for_school_profiles_results_count').html();
    clear_all_fields();
